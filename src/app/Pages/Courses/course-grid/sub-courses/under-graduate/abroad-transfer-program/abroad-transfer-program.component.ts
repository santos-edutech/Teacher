...
declare var $:any;
@Component({
  selector: 'app-abroad-transfer-program',
  templateUrl: './abroad-transfer-program.component.html',
  styleUrls: ['./abroad-transfer-program.component.css'],
  providers: [PaymentService]
})
export class AbroadTransferProgramComponent implements OnInit {
      ...
      if(menuItems.menuId == MenuId){
        ...
        for(let subMenuItem of this.menuItemList){
          if(subMenuItem.id == ID){
            ...
            this.selectedSubjectUniqId = subMenuItem.uniqueID;
 ...
 buyCourse(){
   this.courseItems = this.fb.group({
     uniqueID : this.selectedSubjectUniqId,
